...
# custom callback for PudgyPenguins
...
def folder_from_template_created(path: str, source: str, tokens: dict[str, str]):
    ...
    def replace_and_write(file_path):
        try:
            with open(file_path, 'r') as file:
                content = file.read()

            # Replace "[Name]" with Token
            modified_content = content.replace("[Name]", tokens["Name"])

            with open(file_path, 'w') as file:
                file.write(modified_content)

            print("Replacement complete and content written back to the file.")

        except FileNotFoundError:
            print(f"File not found: {file_path}")
        except IOError:
            print(f"Error reading/writing file: {file_path}")

    replace_and_write(os.path.join(
        path, f'Prp_{tokens["Name"]}', "PALETTE_LIST"))
    replace_and_write(os.path.join(
        path, f'Prp_{tokens["Name"]}', f'Prp_{tokens["Name"]}.aux'))
    replace_and_write(os.path.join(
        path, f'Prp_{tokens["Name"]}', 'scene.versionTable'))
